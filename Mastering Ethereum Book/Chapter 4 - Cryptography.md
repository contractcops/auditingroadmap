...
> ❗ The private key controls the access by being a unique piece when needed to create digital signatures. Those digital signatures are used to "sign" transactions and to spend any funds into the account. The digital signatures also act as a proof of authentication when it comes down to smart contracts or transaction owners.
...
> ❗ This generator point is the same one for all of the users in Ethereum, meaning that a private key multiplied by the generator point will always result in the same public key K. 
